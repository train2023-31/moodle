<?php
// local/financeservices/classes/simple_workflow_manager.php
...
/**
 * Simple Workflow Manager for Finance Services (hard-coded logic).
 *
 * Mirrors the structure of the residence-booking workflow manager so
...
class simple_workflow_manager {
    ...
    public const STATUS_INITIAL        = 8;
    public const STATUS_LEADER1_REVIEW = 9;
    public const STATUS_LEADER2_REVIEW = 10;
    public const STATUS_LEADER3_REVIEW = 11;
    public const STATUS_BOSS_REVIEW    = 12;
    public const STATUS_APPROVED       = 13;
    public const STATUS_REJECTED       = 14;
    ...
    private static function get_workflow_map(): array {
        return [
            self::STATUS_INITIAL => [
                ...
                'capability' => 'local/status:finance_workflow_step1',
            ],
            self::STATUS_LEADER1_REVIEW => [
                'next'       => self::STATUS_LEADER2_REVIEW,
                'capability' => 'local/status:finance_workflow_step2',
            ],
            self::STATUS_LEADER2_REVIEW => [
                'next'       => self::STATUS_LEADER3_REVIEW,
                'capability' => 'local/status:finance_workflow_step3',
            ],
            self::STATUS_LEADER3_REVIEW => [
                'next'       => self::STATUS_BOSS_REVIEW,
                'capability' => 'local/status:finance_workflow_step4',
            ],
     ...
     *
     * Added: fallback to `local/financeservices:manage` so managers/admins
    ...
    public static function can_user_approve(int $sid): bool {
        ...
        $caps[] = 'local/financeservices:manage';
    ...
    public static function approve_request(int $id, string $note = ''): bool {
        ...
        $req = $DB->get_record('local_financeservices', ['id' => $id], '*', MUST_EXIST);
        $cur = $req->status_id;
        ...
        $DB->set_field('local_financeservices', 'status_id', $next, ['id' => $id]);
        $DB->set_field('local_financeservices', 'rejection_note', null, ['id' => $id]);

        if ($note !== '') {
            $DB->set_field('local_financeservices', 'approval_note', $note, ['id' => $id]);
        }
    ...
    public static function reject_request(int $id, string $note): bool {
        ...
        $DB->set_field('local_financeservices', 'status_id',     $next, ['id' => $id]);
        $DB->set_field('local_financeservices', 'rejection_note', $note, ['id' => $id]);
    ...
    private static function log_action(
        ...
        global $USER;
        debugging(
            "FS-WF #{$rid}: {$action} {$from}→{$to} by user {$USER->id}. Note=\"{$note}\"",
            DEBUG_DEVELOPER
        );
    }
